...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImGui::GetColorU32(ImGuiCol_FrameBg), 1.0f); // Use FrameBg color for checkbox background
    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImGui::GetColorU32(ImGuiCol_CheckMark), it_anim->second.selected_rect); // Use CheckMark color for checkbox fill

    // Render dot instead of check mark
    if (*v)
        window->DrawList->AddCircleFilled(ImVec2(total_bb.Min.x + square_sz * 0.5f, total_bb.Min.y + square_sz * 0.5f), square_sz * 0.5f * 0.5f, ImGui::GetColorU32(ImGuiCol_Text), 12); // Dot
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, float size)
{
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y - 3.0f), label);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    PushStyleColor(ImGuiCol_FrameBg, ImVec4(0.10f, 0.10f, 0.10f, 0.30f));
    PushStyleColor(ImGuiCol_FrameBgHovered, ImVec4(0.08f, 0.08f, 0.08f, 0.30f));
    PushStyleColor(ImGuiCol_FrameBgActive, ImVec4(0.08f, 0.08f, 0.08f, 0.30f));
    ...
    if ((flags & ImGuiColorEditFlags_DisplayRGB) && !(flags & ImGuiColorEditFlags_NoInputs))
    {
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleColor(ImGuiCol_PopupBg, ImVec4(0.05f, 0.05f, 0.05f, 1.00f));
