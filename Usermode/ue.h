...
public:
	...
	inline void normalize(fvector& in) {
		SPOOF_FUNC
			in.x = std::fmod(in.x + 180.0f, 360.0f);
		if (in.x < 0.0f) in.x += 360.0f;
		in.x -= 180.0f;

		in.y = std::fmod(in.y + 180.0f, 360.0f);
		if (in.y < 0.0f) in.y += 360.0f;
		in.y -= 180.0f;

		in.z = 0.0f;
	}

	// Calculate pitch and yaw angles between two positions
	inline fvector calc_angle(fvector LocalPos, fvector WorldPos) {
		SPOOF_FUNC
			fvector RelativePos = WorldPos - LocalPos;
		float yaw = atan2f(RelativePos.y, RelativePos.x) * 180.0f / M_PI;
		float pitch = -((acosf(RelativePos.z / LocalPos.distance(WorldPos)) * 180.0f / M_PI) - 90.0f);

		return fvector(pitch, yaw, 0.0f);
	}

	// Check if a screen location is within the screen bounds
	inline bool in_screen(fvector2d screen_location) {
		SPOOF_FUNC
			return (screen_location.x > 0 && screen_location.x < screen_width && screen_location.y > 0 && screen_location.y < screen_height);
	}
};
...
inline void DrawCorneredBox(int x, int y, int w, int h, const ImU32& color, int thickness, int borderPx, bool outlined) {
	...
	if (outlined) {
		...
		auto drawColor = ImGui::ColorConvertFloat4ToU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f));

		// Draw top outline
		drawList->AddLine(ImVec2(x + cornerBoxLength, y), ImVec2(x, y), drawColor, thickness * 2);
		drawList->AddLine(ImVec2(x, y + cornerBoxHeight), ImVec2(x, y), drawColor, thickness * 2);
		drawList->AddLine(ImVec2(x + w - cornerBoxLength, y), ImVec2(x + w, y), drawColor, thickness * 2);
		drawList->AddLine(ImVec2(x + w, y + cornerBoxHeight), ImVec2(x + w, y), drawColor, thickness * 2);

		// Draw bottom outline
		drawList->AddLine(ImVec2(x + cornerBoxLength, y + h), ImVec2(x, y + h), drawColor, thickness * 2);
		drawList->AddLine(ImVec2(x, y + h - cornerBoxHeight), ImVec2(x, y + h), drawColor, thickness * 2);
		drawList->AddLine(ImVec2(x + w - cornerBoxLength, y + h), ImVec2(x + w, y + h), drawColor, thickness * 2);
		drawList->AddLine(ImVec2(x + w, y + h - cornerBoxHeight), ImVec2(x + w, y + h), drawColor, thickness * 2);
	}

	auto drawList = ImGui::GetForegroundDrawList();
	auto drawColor = ImGui::GetColorU32(color);

	// Draw corner lines
	drawList->AddLine(ImVec2(x + cornerBoxLength, y), ImVec2(x, y), drawColor, thickness);
	drawList->AddLine(ImVec2(x, y + cornerBoxHeight), ImVec2(x, y), drawColor, thickness);
	drawList->AddLine(ImVec2(x + w - cornerBoxLength, y), ImVec2(x + w, y), drawColor, thickness);
	drawList->AddLine(ImVec2(x + w, y + cornerBoxHeight), ImVec2(x + w, y), drawColor, thickness);
	drawList->AddLine(ImVec2(x + cornerBoxLength, y + h), ImVec2(x, y + h), drawColor, thickness);
	drawList->AddLine(ImVec2(x, y + h - cornerBoxHeight), ImVec2(x, y + h), drawColor, thickness);
	drawList->AddLine(ImVec2(x + w - cornerBoxLength, y + h), ImVec2(x + w, y + h), drawColor, thickness);
	drawList->AddLine(ImVec2(x + w, y + h - cornerBoxHeight), ImVec2(x + w, y + h), drawColor, thickness);
}
